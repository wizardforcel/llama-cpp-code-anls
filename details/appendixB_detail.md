...
GBNF（GGML BNF）是 llama.cpp 使用的语法约束格式，基于扩展的巴科斯-瑙尔范式（EBNF）。它允许开发者定义严格的输出格式规则，确保模型生成符合预期的结构化数据。
...
### B.4.2 懒加载语法（Lazy Grammar）

懒加载语法允许在特定触发条件满足后才激活语法约束：
...
// 初始化懒加载语法采样器
struct llama_sampler * smpl = llama_sampler_init_grammar_lazy_patterns(
    vocab,
    json_grammar,           // JSON语法
    "root",                 // 根规则
