...
| 参数 | 说明 | 示例 |
|-----|------|------|
| `-m, --model` | 模型文件路径 | `-m models/llama-7b.gguf` |
| `-p, --prompt` | 输入提示词 | `-p "Hello world"` |
| `-n, --n-predict` | 生成token数量 | `-n 256` |
| `-cnv, --conversation` | 交互对话模式 | `-cnv` |
| `-t, --threads` | CPU线程数 | `-t 8` |
| `-ngl, --n-gpu-layers` | GPU加载层数 | `-ngl 35` |
