...
src/llama-vocab.h
├── llama_vocab              # 词表主类
│   ├── token_data           # Token数据结构
│   ├── llama_vocab_pre_type # 预分词类型枚举
│   └── 核心API
│       ├── tokenize()       # 文本→tokens
...
├── BPE分词 (第1000-2000行)  # byte-pair encoding
├── SPM分词 (第2000-3000行)  # SentencePiece
├── 预分词  (第3000-4000行)  # 正则切分
...
### 练习2：分析预分词正则
阅读 `src/unicode.cpp` 第214-330行，回答：
1. GPT-2正则如何处理缩写（如"don't"）？
2. 数字是如何被切分的？
3. 空格处理有什么特殊之处？
