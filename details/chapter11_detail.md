...
src/llama-memory.h
├── llama_memory_params      # 内存参数结构
│   ├── type_k               # K缓存数据类型
│   └── type_v               # V缓存数据类型
├── llama_memory_status      # 内存状态枚举
│   ├── SUCCESS              # 成功
│   ├── NO_UPDATE            # 无需更新
│   ├── FAILED_PREPARE       # 准备失败
│   └── FAILED_COMPUTE       # 计算失败
├── llama_memory_context_i   # 内存上下文接口（处理批次）
│   ├── next()               # 切换到下一批次
│   ├── apply()              # 应用内存更新
│   └── get_ubatch()         # 获取当前批次
└── llama_memory_i           # 内存管理接口基类
    ├── init_batch()         # 初始化批次处理
    ├── init_full()          # 初始化满缓存
    ├── init_update()        # 初始化更新
    ├── seq_rm/cp/keep/add/div()  # 序列操作
    └── state_write/read()   # 状态读写
```
...
在深入了解KV缓存之前，我们需要先理解`llama-memory.h`定义的**内存管理抽象接口**。这是llama.cpp中所有内存类型（KV缓存、循环状态缓存等）的统一基类。
...
1. **统一接口**：所有内存类型（KV缓存、循环状态、混合内存）都实现`llama_memory_i`，使得`llama_context`可以用相同的方式处理不同类型的模型

2. **批次处理抽象**：`init_batch()`返回`llama_memory_context_i`，用于迭代处理分割后的ubatches

3. **状态持久化**：统一的`state_write/read`接口支持会话保存/恢复
