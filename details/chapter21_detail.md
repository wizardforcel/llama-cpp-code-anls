...
2. 深入MTMD（Multi-Modal Text-Image Decoder）架构的实现细节
...
- **图像分块** = 观察照片的细节（先看整体，再看局部）
- **投影层** = 翻译官（将视觉特征翻译成文本模型能理解的格式）

就像人类需要眼睛、大脑和语言能力的协同工作才能"看图说话"，多模态模型也需要视觉编码器、投影层和语言模型的紧密配合。
...
**关键设计决策**：
1. **统一媒体标记**：使用`<__media__>`作为统一的图像/音频标记
2. **可插拔编码器**：支持视觉编码器（CLIP）和音频编码器
3. **分块处理**：支持大图像的分块编码（llava-uhd风格）
...
阅读 `tools/mtmd/mtmd.cpp` 第1-300行，回答：
1. `mtmd_context` 结构体包含哪些成员？
2. 视觉编码器和音频编码器如何共存？
3. `slice_tmpl` 如何根据模型自动选择？
...
|------|-----------|
| MTMD | 多模态文本-图像解码器架构 |
| CLIP | 视觉编码器，将图像转为特征向量 |
| 投影层 | 对齐视觉特征和文本嵌入空间 |
| 图像分块 | 大图像切分处理，平衡细节与效率 |
| `<__media__>` | 统一媒体标记，标识图像位置 |
| M-RoPE | 多模态位置编码，处理2D图像位置 |
| KV缓存复用 | 相同图像缓存视觉特征，加速后续查询 |
