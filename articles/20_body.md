...
- **控制向量**：调音台上的各个旋钮（低音增强、高音提升、混响效果、回声延迟等），每个旋钮对应一个特定效果

- **正向样本**："开心"的语音片段 → 提取出"开心"向量的方向

- **负向样本**："悲伤"的语音片段 → 提取出"悲伤"向量的方向

- **向量运算**：开心向量 - 悲伤向量 = "更开心"的控制向量，放大了两种情感之间的差异

- **应用控制**：将控制向量加到隐藏层，就像调节调音台旋钮，改变模型的"语气"

**与LoRA的区别**：
- LoRA像换一个人来唱（改变权重矩阵，改变了模型的"声带结构"）
- 控制向量像给同一个人加音效（改变激活值，只是改变了"声音效果"）
...
| 方法 | 优点 | 缺点 |
|------|------|------|
| **PCA** | 找到最大方差方向，去噪效果好 | 计算复杂，需要迭代 |
| **Mean** | 简单快速 | 可能包含噪声方向 |
| **Median** | 鲁棒性好，对异常值不敏感 | 可能丢失一些信息 |

### 20.6.2 层范围选择

| 层范围 | 影响层面 | 效果特点 |
|--------|----------|----------|
| 浅层（0-10） | 词法、语法 | 影响措辞和句式 |
| 中层（10-20） | 语义、风格 | 影响表达方式和语气 |
| 深层（20+） | 整体连贯性 | 影响逻辑和一致性 |
| 全部层 | 所有层面 | 最强效果，但可能不稳定 |

### 20.6.3 样本数量与质量

- **样本数量**：通常10-50对样本足够，过多可能引入噪声
- **样本质量**：正负样本对比要鲜明，差异要清晰
- **样本多样性**：覆盖目标行为的不同表现，避免过拟合
...
| 概念 | 一句话总结 |
|------|-----------|
| **控制向量** | 通过修改隐藏层激活值控制生成风格的轻量级技术 |
| **对比学习** | 正向样本 - 负向样本 = 控制方向 |
| **PCA降维** | 提取差异矩阵的主成分方向 |
| **广播加法** | 控制向量 [1, n_embd] 加到激活 [n_tokens, n_embd] |
| **层范围控制** | 可选择性应用到浅层/中层/深层 |
| **强度调节** | scale参数控制效果强弱，可为负值反向 |
...
在第21章，我们将探索多模态支持——让llama.cpp不仅能够处理文本，还能理解图像，实现图文对话能力。
