...
当你向朋友描述一张照片时，你的视觉系统、语言中枢和发声器官需要完美配合才能完成这个任务。你的眼睛就像CLIP视觉编码器，将光线信息和像素信号转换为大脑能够理解的"视觉语言"——这是一系列神经元的激活模式，编码了图像的内容、结构和语义。你的大脑相当于大语言模型，它将这些视觉信号与储存在记忆中的知识和经验相结合，进行理解和推理：这个场景发生在哪里？人们正在做什么？这传递了什么情感？最后，你的嘴巴——也就是文本生成器——将这些理解和推理转化为流畅的语言，形成一句句有意义的描述。

在这个过程中，图像分块处理对应了我们观察照片时的自然行为：先扫一眼整体获得大致印象（缩略图），再仔细查看各处的细节（分块）。我们不会将一张大照片的所有细节同时映入脑海，而是有策略地分配注意力。投影层则是这套系统中的翻译官，它负责将视觉编码器输出的"视觉语言"翻译成语言模型能理解的"文本语言"。毕竟，在这个比喻中，眼睛说的是视觉信号，而大脑处理的是语义符号，中间必须有可靠的翻译机制。

就像人类需要眼睛、大脑和语言能力的协同工作才能完成"看图说话"，多模态模型也需要视觉编码器、投影层和语言模型的紧密配合。如果眼睛看到的视觉信息不能正确传达给大脑，或者大脑理解了却无法用语言表达出来，整个看图说话的系统就无法正常工作。这就是为什么MTMD架构中每个组件的设计都如此关键。
...
MTMD面临一个核心架构选择：是将视觉编码和语言模型合并为一个端到端的联合模型，还是将它们分离为两个独立模块通过投影层连接。端到端联合模型在理论上可能达到最优性能，因为视觉和语言部分可以端到端地联合训练和优化。但这种方案灵活性极差：一旦模型训练完成，很难替换其中任何一个组件，而且不同的视觉编码器和语言模型组合都需要重新训练。纯文本模型外挂虽然简单，只给语言模型增加一个视觉预处理步骤，但性能往往很差，因为语言模型本身没有被训练来理解视觉特征。

llama.cpp选择了分离式架构，这是经过深思熟虑的选择。分离式架构的第一个优势是模块化——CLIP视觉编码器和语言模型可以独立更新，一个组件的改进不会影响另一个。第二个优势是灵活性——同一个视觉编码器（如CLIP ViT-L/14）可以搭配不同大小和种类的语言模型（Llama、Mistral等），而无需重新训练视觉部分。第三个优势是效率——对于相同或相似图像的反复查询，视觉特征可以被缓存和复用，避免了重复的视觉编码计算。分离式架构的唯一代价是需要一个投影层来对齐视觉特征空间和文本嵌入空间，但这个投影层（存储在mmproj文件中）参数量极小，对整体性能几乎没有影响。

### 为什么使用分块（Slice/Tiles）处理大图像？

单张大图像直接编码会产生大量token，例如一张1024x1024的图像直接编码会生成约1024个token，这不仅占用了大量的KV缓存空间，还可能超出模型的上下文长度限制。分块处理通过将大图像切分为多个小块来平衡细节和效率：一个典型的2x2分块策略会产生一个全局缩略图加上4个分块，总共约720个token。这既保留了高分辨率的局部细节（通过分块），又保持了整体的图像理解（通过缩略图）。更重要的是，分块的数量和大小可以根据图像原始尺寸和可用的显存进行灵活调整，这使得MTMD可以处理从手机快照到高分辨率扫描文档等各种尺寸的图像，而不会因token数量过多而导致推理失败。
...
| 概念 | 解释 |
|------|------|
| MTMD架构 | 多模态文本-图像解码器架构，通过CLIP视觉编码器+投影层+语言模型串联实现图文理解 |
| CLIP视觉编码器 | 基于Vision Transformer的视觉特征提取器，将图像编码为固定维度的特征向量 |
| 图像分块处理 | 将大图像切分为缩略图+多个分块，在保留细节的同时控制token数量 |
| 投影层（mmproj） | 对齐视觉特征空间和文本嵌入空间的桥梁，存储在mmproj文件中 |
| 分块模板系统 | 针对不同模型（LLaMA 4、MiniCPM-V等）的标记格式差异提供适配层 |
| M-RoPE位置编码 | 专门处理2D图像位置信息的位置编码方式，用于区分空间位置关系 |

下一章中，我们将学习llama.cpp的Common Utilities——了解CLI工具背后的共享基础设施，包括参数解析、下载管理、控制台交互等实用功能。
