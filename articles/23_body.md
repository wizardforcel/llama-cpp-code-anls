...
想象你走进一家高档餐厅，面对一本厚厚的菜单却不知从何下手。这时，一个智能点餐助手来到你身边，它要根据你的喜好、预算、饮食限制，从上百道菜中为你推荐最合适的选择。这个推荐过程，与大语言模型的采样机制惊人地相似。菜单上的每一道菜都对应模型词汇表中的一个Token——当模型生成文本时，它会为每个Token计算一个logit分数，就像每道菜都有一个综合评分，综合考量了食材新鲜度、厨师擅长程度和顾客评价等因素。这些评分反映了在当前上下文中每个Token的合适程度。

温度参数就像你告诉推荐系统的"冒险程度"。如果温度很低（如0.2），你会说"请推荐最稳妥的招牌菜"——系统总是选择评分最高的选项，结果可预测但缺乏惊喜。如果温度很高（如1.5），你会说"我想尝试些新鲜刺激的"——系统更愿意推荐评分略低但独特的创意菜品，结果更多样化但也可能踩雷。Top-K采样则像是直接限制"只给我看评分最高的前K道菜"，这确保了推荐质量不会太差，但K值太小可能错过隐藏的好选择，太大则低质量选项又会混入。Top-P采样更聪明一些：它列出能让你90%满意的菜品集合，按评分从高到低累加概率直到达到P值，只在这个动态大小的集合中随机选择。

重复惩罚机制处理的是"审美疲劳"问题：你已经连续三天吃了红烧肉，今天再看到它时，即使评分很高也想换换口味。最近出现过的Token被降低概率，避免了模型像复读机一样重复同样的词语。语法约束则对应着饮食限制：素食者的菜品必须不含肉类，对坚果过敏的人必须排除含坚果的菜品。即使某个Token概率很高，如果它违反了语法规则（如JSON中缺少引号），也会被无情排除。一个优秀的点餐系统不会只做一次筛选——它可能先根据饮食限制过滤，再按评分排序，再考虑价格区间，最后加入随机性增加惊喜。采样器链正是这样工作的：多个采样策略按顺序应用，每个策略都在前一个的基础上进一步精炼候选集。通过这个类比我们可以看到，采样不仅仅是"随机选一个Token"那么简单，而是一个复杂的多阶段决策过程，需要在创造性、连贯性和约束条件之间找到平衡。
...
llama.cpp的采样器在处理语法约束时采用了一种聪明的两阶段策略，而非总是先应用语法约束。这个设计基于一个重要的观察：在绝大多数情况下（90%以上），采样器选出的token本身就符合当前语法状态。想象你正在生成JSON，当前处于`"name": "`之后——模型大概率会生成正常的字符串内容，而不会突然输出`}`或`{`。因此，总是先应用完整的语法约束（需要检查词汇表中所有50000+个token）是一种浪费。

快速路径的策略是"先采样，后检查"：正常应用采样器链选择token，然后快速验证这个token是否符合语法。对于90%以上的情况，token天然符合语法，直接返回即可，额外开销极小（只有一次单token验证）。当token不符合语法时，快速路径失败，此时才会走慢速路径：重新获取原始logits，先应用语法约束将不合法的token概率置零，再通过采样器链重新选择。快速路径的平均性能接近无语法检查，最坏情况下延迟加倍（需要两次采样流程和完整的语法检查），但这种情况发生概率很低。这个策略的代价是代码复杂度增加了——需要维护两条不同的执行路径，以及需要额外的临时存储来保存候选数组的副本（防止快速路径的修改影响慢速路径的恢复）。但考虑到正确性需求（语法约束在结构化输出中是必需的）和性能需求（大部分情况不需要语法约束干预），这个权衡是非常值得的。

### 为什么采样器链使用动态数组而不是链表？

llama.cpp的采样器链使用`std::vector<llama_sampler *>`作为底层数据结构而非链表。动态数组在这个场景下有多个优势：首先，缓存友好是最重要的考量——采样器指针连续存储在内存中，遍历时CPU缓存命中率极高，而链表的每个节点分散在堆上，每次访问下一个节点都可能触发缓存未命中。其次，内存开销更小——动态数组只需存储指针本身，而链表每个节点还需要额外存储一个next指针（在64位系统上额外8字节），对于采样器链这种通常只有5-10个元素的短列表，链表的额外开销占比很高。第三，虽然采样的核心操作是顺序遍历（依次应用各个采样器），但偶尔也需要通过索引访问特定位置的采样器，例如性能统计输出时需要打印链中第N个采样器的名称。链表的主要优势——O(1)的中间插入和删除——在这个场景下几乎用不到，因为采样器链在初始化后很少被修改，它是一个"创建后不变"的数据结构。因此，动态数组是综合考量了性能、内存和简单性之后的自然选择。
...
| 概念 | 解释 |
|------|------|
| common_sampler | 封装llama_sampler链的高层采样器，管理历史缓冲区、候选缓存和性能统计 |
| ring_buffer | 固定大小的环形缓冲区模板，替代std::deque以O(1)实现滑动窗口Token历史 |
| 采样器链顺序 | 惩罚采样器最先→温度缩放→Top-K/Top-P/Min-P过滤→分布采样器最后，顺序不可随意调换 |
| 快速路径+慢速路径 | 两阶段语法检查策略：90%+情况走快速路径（先采样再验证单token），少数走慢速路径（先语法再采样） |
| RAII时间测量 | common_time_meas利用构造/析构自动计时，消除手动管理错误，线程安全 |
| Mirostat | 自适应采样技术，动态调整内部温度以维持目标困惑度，与普通dist采样互斥 |

下一章中，我们将学习llama.cpp的命令行工具详解——深入llama-cli、llama-server、llama-quantize和llama-bench的实现和使用。
