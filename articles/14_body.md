...
| 概念 | 一句话总结 |
|------|-----------|
| **llm_chat_template** | 聊天模板类型枚举，定义所有支持的对话格式 |
| **llm_chat_detect_template** | 通过特征检测模板类型，支持从Jinja2模板字符串识别 |
| **llm_chat_apply_template** | 将消息列表格式化为模型可理解的提示文本 |
| **ChatML** | 通用模板，使用`<|im_start|>`和`<|im_end|>`标记 |
| **Llama3** | 结构化模板，使用`<|start_header_id|>`和`<|eot_id>` |
| **add_ass** | 是否在末尾添加assistant标记，提示模型开始生成 |
| **LU8宏** | 处理UTF-8字符串字面量，用于中文模板标记 |

**聊天模板系统的设计哲学**：

1. **零依赖**：不引入Jinja2等外部库，保持llama.cpp的轻量级特性
2. **启发式检测**：通过特征字符串匹配实现高效的模板识别
3. **精确实现**：为每种模板硬编码格式化逻辑，确保输出正确
4. **灵活扩展**：通过枚举和switch-case支持新模板的添加

就像裁缝需要了解每种面料的特性才能做出合身的衣服，聊天模板系统需要深入理解每种模型的对话格式才能产生正确的提示。这种对细节的关注是llama.cpp能够支持数百种模型的关键。
