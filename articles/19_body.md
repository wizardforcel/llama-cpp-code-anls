...
| 概念 | 一句话总结 |
|------|-----------|
| **LoRA** | 低秩分解，ΔW = B × A，只训练0.78%参数 |
| **缩放因子** | scale = alpha / rank，控制适配强度 |
| **GGUF LoRA** | 标准化适配器格式，包含metadata和张量对 |
| **张量配对** | lora_a (r×k) + lora_b (d×r) 组成完整LoRA |
| **多适配器** | 可加权组合多个适配器，实现能力叠加 |
| **aLoRA** | 通过特定token触发的条件适配器 |
...
1. **rank选择**：从r=16开始，根据效果调整
2. **alpha设置**：通常alpha = rank或2×rank
3. **scale调整**：推理时通过scale微调效果强度
4. **多适配器**：用scale实现平滑的能力插值
