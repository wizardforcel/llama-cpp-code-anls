...
当你需要仔细观察模型如何处理文本时，分词工具就像是一枚高倍放大镜。你可以看清每一个token的边界，理解为什么"ChatGPT"被分成"Chat"和"GPT"，或者为什么中文"你好世界"变成了四个token。在调试聊天模板或处理特殊字符时，这个"放大镜"能让你发现肉眼看不见的细节。

**llama-embedding = 测量仪**

嵌入向量是文本的数学指纹。有了嵌入工具，你就像拥有了一台精密的测量仪，可以将任何文本转换成一个高维向量，然后计算它们之间的距离。"国王"和"女王"的向量很接近，但"国王"和"苹果"就很远——这种语义关系的量化正是RAG（检索增强生成）系统的核心。

**llama-perplexity = 质检仪**

在工厂里，质检仪确保每件产品都符合标准。困惑度工具就是你的"模型质检仪"——它告诉你模型对测试数据的预测有多"困惑"。量化后的模型体积小了，但质量下降了多少？困惑度给你一个量化的答案。

**gguf-split/gguf = 切割刀和标签机**

当你的模型文件太大，无法上传到网盘或内存不足时，GGUF分割工具就像一把精准的切割刀，将大文件切分成小块。而GGUF查看工具则像标签机，让你清晰了解模型文件里有什么、每个张量多大、什么格式。

**convert_lora_to_gguf.py = 适配器转换器**

在HuggingFace上找到一个好用的LoRA适配器，但它是PyTorch格式？这个转换器就像是电源适配器，让不同格式的模型能够在llama.cpp的生态中工作。
...
| 方案 | 优点 | 缺点 | 选择 |
|-----|------|------|------|
| 单一工具 | 统一接口，学习成本低 | 体积大，启动慢，加载无关代码 | 否 |
| 分离工具 | 专注高效，可组合 | 学习成本稍高 | **是** |
| 库+脚本 | 灵活，可编程 | 需要运行时环境 | 部分使用 |

**分离工具的优势**：

1. **启动快**：只加载需要的代码
   ```bash
   # llama-tokenize启动时间 < 100ms
   # 如果包含所有工具，可能需要 > 500ms
   ```
...
3. **可组合**：通过shell管道组合
   ...
   # 复杂工作流示例
   cat input.txt | \
       ./llama-tokenize -m model.gguf --ids | \
 ...
 * 
 * Python转换的优势：
 * 1. 直接读取PyTorch checkpoint
 * 2. 使用transformers库处理配置
 * 3. 快速迭代和调试
 * 4. 社区熟悉，易于贡献
 * 
 * 架构决策：
