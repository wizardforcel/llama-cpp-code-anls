...
想象你在写一篇长文章。普通的自回归生成就像逐字逐句地写，每写一个字都要深思熟虑——质量高但速度慢得让人着急。投机解码则像先快速草拟一整段（用草稿模型），然后一次性校对（用目标模型）——如果草稿质量不错，一次校对就验证了多句，大大节省了时间；如果某句草稿不太好，就只重写那一句。关键就在于草稿要快、校对要批量化。

前瞻解码换个方式提速：同时写多个版本的开头，看哪个版本能延续出更长的正确内容——就像同时尝试多条故事线，哪条发展最顺畅就选哪条。查找解码则更加直接：从之前写过的文章中找相似的段落直接复用，就像有经验的写手常说"这个我之前写过，复制过来改改就行"。

这些技术的核心思想是一样的：减少大模型的调用次数。无论是"小模型快速猜测 + 大模型批量验证"，还是"并行尝试多路 + 选择最优路径"，目标都是让大模型少做几次推理，因为大模型推理一次的时间远大于所有其他操作。
...
不同的加速技术在额外资源需求和加速效果上存在显著差异。草稿模型投机需要加载完整的小模型，额外显存要求最高，但加速效果也最好（1.5-2.5x），适合显存充足的场景。前瞻解码虽然不需要额外模型，但需要维护 W+G+1 倍的 KV 缓存，加速效果中等（1.3-2.0x），适合 KV 缓存充裕的批量推理。查找解码只需要磁盘上的预建缓存文件，额外内存最小，但加速效果也较低（1.1-1.5x），适合固定领域的重复性内容。N-gram 自投机介于两者之间，仅需维护 N-gram 词表，无需草稿模型，适合重复内容较多的场景。选择时首先问自己：有没有合适的小模型？有则选草稿模型投机，没有则根据文本重复性选择 N-gram 投机或查找解码。

### 如何平衡生成质量与加速效率？

投机解码存在一个核心的权衡参数：草稿接受率。高接受率策略保守生成草稿——只生成置信度很高的 token，这样草稿几乎都会被接受，但草稿序列短，整体加速效果有限。高投机率策略激进生成草稿——用较低的置信度阈值生成更多草稿 token，虽然可能被拒绝的多，但如果接受率高总体吞吐量更优。最佳平衡点一般在接受率 60-80% 时，草稿既不会太保守也不会太激进，整体效率达到最优。

### 这些技术的实现复杂度如何？

按实现难度从低到高排列：查找解码最简单，本质上就是预建 N-gram 缓存的查询匹配。N-gram 自投机稍复杂，需要维护状态机并实时更新 N-gram 表。草稿模型投机进一步复杂，涉及双模型管理、KV 缓存同步、接受/拒绝概率计算等。前瞻解码最复杂，需要构建复杂的批处理来同时管理 Jacobi 迭代位置和多组验证 N-gram，批处理调度逻辑尤为困难。对于实际部署，草稿模型投机虽然不简单但性价比最高，llama.cpp 也将其实现封装成了方便调用的 API。
...
本章深入解析了多种解码加速技术。草稿模型投机采用小模型猜测、大模型验证的策略，通过减少大模型调用次数实现加速，适用于有小模型可用的场景。N-gram自投机利用模型自身的历史模式进行投机，利用文本的重复性特点提升速度。前瞻解码通过Jacobi迭代并行化技术，同时验证多条解码路径，适用于KV缓存充足的场景。查找解码通过预建缓存查询，跳过重复计算，适用于领域固定的场景。

选型决策树：是否有合适的小模型？如果有，选择草稿模型投机效果最好；如果没有，需要评估文本重复性，重复性高选择N-gram投机或查找解码，重复性低选择前瞻解码。

关键经验包括：草稿质量是关键，草稿模型不需要很强但速度要快；接受率在60-80%时最优，太低则草稿无效，太高则草稿生成太慢；硬件资源决定上限，显存大小决定了可以同时维护多少序列；领域适配很重要，通用场景用通用草稿模型，专业场景用专业缓存。

本章我们一起学习了以下概念：

| 概念 | 解释 |
|------|------|
| 投机解码 | 用小模型快速生成草稿，大模型并行验证并接受匹配的 token |
| 接受率 | 草稿 token 被目标模型接受的比例，60-80% 时整体效率最优 |
| N-gram 自投机 | 基于模型自身历史输出构建 N-gram 表，无需额外草稿模型 |
| 前瞻解码 | 基于 Jacobi 迭代并行验证多个位置的 token，一次 decode 测试多条路径 |
| 查找解码 | 从预建的 N-gram 缓存中查找历史模式，跳过重复计算 |
| KV 缓存复用 | 投机解码中草稿模型复用目标模型的部分 KV 缓存，减少重复计算 |

**下一章预告**：

下一章中，我们将学习 LoRA 适配器支持，理解如何以极小的成本微调模型，实现个性化定制和特定领域优化。
