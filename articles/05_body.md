...
- **原始 FP32 模型** = 每本书都用高清彩色印刷（4 字节/字），质量最高但占空间大
- **量化压缩** = 将书籍转为精简版：
  - **Q8_0** = 高保真扫描（1 字节/字，质量接近原版，适合珍贵藏书）
  - **Q4_0** = 精要摘录（4 比特/字，体积 25%，质量可接受，适合普通阅读）
  - **Q4_K_M** = 智能摘要（重要章节保真，次要章节精简，平衡之选）
- **量化块（block）** = 每 64 页为一组，共用一套"缩放说明书"
- **反量化** = 阅读时根据说明书还原内容到可阅读格式
- **IQ 系列** = 智能压缩算法（根据内容重要性动态调整精度）
...
```
原始 FP32 数据（32 个元素）: 128 字节
[0.5] [-0.3] [1.2] [-0.8] ... [0.8]
  ↓ 量化（找到最大绝对值，计算 scale）
  ↓ max(|x|) = 1.2, scale = 1.2 / 7 ≈ 0.171
...
│  4 字节     │  1 字节│ 1 字节│     │ 1 字节      │
├─────────────────────────────────────────────────┤
│  scale≈0.17 │w0|w1  │w2|w3  │ ... │w30|w31      │
│             │4b|4b  │4b|4b  │     │4b |4b       │
└─────────────────────────────────────────────────┘

量化值计算：
  w0 = round(0.5 / 0.171) = 3  → 存储为 3 + 8 = 11 (0xB)
  w1 = round(-0.3 / 0.171) = -2 → 存储为 -2 + 8 = 6 (0x6)
  
qs[0] = (w1 << 4) | w0 = (0x6 << 4) | 0xB = 0x6B

反量化：
  w0 = (qs[0] & 0xF) - 8 = 11 - 8 = 3
  w0_f32 = 3 * 0.171 = 0.513 ≈ 原始 0.5
```
...
```
普通 Q4_0：32 元素共享 1 个 scale
Q4_K_M：256 元素分层量化
  - 每 32 元素有小 scale
  - 整组 256 元素有大 scale
  - 重要权重（如 attention）用更高精度
```
...
| 概念 | 一句话总结 |
|------|-----------|
| **Q4_0** | 4 位对称量化，32 元素/块，压缩比 7x，体积 18 字节/块 |
| **Q4_1** | 4 位非对称量化，多存储 min 值，适合不对称分布 |
| **block_q4_0** | 量化块结构：scale(float) + 16 字节打包权重 |
| **quantize_row** | 找 max 算 scale，映射到[-7, 7]，打包存储 |
| **vec_dot_q4_0** | 整数点积 + 延迟 scale，效率最高的量化计算方式 |
| **K-quants** | 分层量化策略，更大块 + 复杂 scale 分配，质量更高 |
| **IQ 系列** | 智能混合精度，重要权重高精度，极致压缩 |

**量化选择建议**：

- **服务器/追求精度**：Q8_0 或 Q6_K
- **桌面/平衡之选**：Q4_K_M（推荐！）
- **笔记本/轻度使用**：Q4_0
- **手机/嵌入式**：IQ2_XXS 或 Q2_K

**下一步预告**：

在掌握了量化技术后，我们将在第 6 章深入 GGML 后端系统——理解异构计算的抽象架构和 CPU/GPU 后端的工作原理。
