...
想象 GGML 是一家跨国制造集团，在全球各地运营着不同的工厂。集团总部（Backend 接口）负责制定统一的运营标准和管理流程，确保全球所有工厂都遵循相同的生产规范。各地的具体工厂则根据本地区的资源特色和产业优势进行本地化生产。

在这些工厂中，CPU 工厂就像是一间勤劳全能的通用厂房——在任何地方都能开工，适应性最强，但并非在特定任务上最优。CUDA 工厂则坐落在 NVIDIA 特区，拥有一条超级流水线，吞吐量最大，能火力全开地处理大批量订单。Metal 工厂是 Apple 生态中的精品车间，以最优的能效比著称，精致高效。

集团还设有物流调度中心（Backend 调度器），它根据每笔订单的特点——是适合大规模批量生产还是需要精细化加工——将任务分配到最合适的工厂。而标准集装箱（ggml_tensor）则是统一的货物包装规格，无论货物发往哪个工厂，对方都能准确识别和处理。

就像跨国公司需要在统一标准和因地制宜之间找到平衡一样，GGML 的后端系统也需要在统一接口和发挥各平台优势之间取得最优配置。这个类比将贯穿本章。
...
本章深入剖析了 GGML 的后端抽象架构。`ggml_backend_i` 定义了后端接口，使用 C 语言模拟虚函数表实现多态机制。`supports_op` 函数用于查询后端是否支持特定算子，这是调度决策的重要依据。CPU 后端采用 SIMD 指令集和多线程技术，具有通用性但非最优，适合处理小任务。CUDA 后端基于 cuBLAS 库实现异步执行，吞吐量最高，特别适合大矩阵运算。Metal 后端利用 Unified Memory 架构，能效比最优，是 Apple 设备的最佳选择。后端调度器能够自动选择最佳后端，在性能和内存之间取得平衡。`n_gpu_layers` 参数支持模型分层策略，可以灵活分配 CPU 和 GPU 之间的负载。

本章我们一起学习了以下概念：

| 概念 | 解释 |
|------|------|
| ggml_backend_i 接口 | C 语言函数指针表实现的多态机制，统一 CPU/CUDA/Metal 等后端 |
| SIMD 指令集优化 | 单指令多数据并行，AVX2 一次处理8个float，理论加速8倍 |
| CUDA 异步执行 | 利用 cuBLAS 和 CUDA stream 实现 GPU 非阻塞计算，吞吐量最高 |
| Metal Unified Memory | Apple Silicon 的 CPU/GPU 共享内存架构，零拷贝数据传输 |
| 后端调度器 | 自动评估各后端的算子支持和执行成本，选择最优硬件执行 |
| n_gpu_layers 分层策略 | 将模型前N层在 GPU 执行，其余在 CPU，灵活平衡速度和显存 |

下一章中，我们将学习 llama.cpp 支持的模型架构——理解如何支持 50+ 种不同的 LLM 架构。
