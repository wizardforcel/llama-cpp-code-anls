...
- **命令行解析（arg.cpp）** = 点餐系统（理解顾客的各种特殊要求，如"不要辣"、"加双份芝士"）
- **网络下载（download.cpp）** = 供应链系统（从仓库自动获取食材，确保不断货）
- **控制台交互（console.cpp）** = 服务员对讲机（实时沟通，处理突发情况）
- **日志系统（log.cpp）** = 后厨监控（记录一切操作，便于排查问题和改进流程）
- **聊天处理（chat.cpp）** = 菜单翻译（将顾客需求转化为厨师能理解的指令）

就像一家高效餐厅需要完善的后厨支持，一个好的LLM工具也需要common库这样的基础设施。没有它，每个工具都要重复实现相同的功能，既低效又容易出错。
...
在第23章，我们将探索高级采样工具的实现——了解common/sampling.cpp如何将llama-sampler封装为更易用的API。
