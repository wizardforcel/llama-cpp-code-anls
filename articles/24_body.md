...
想象llama.cpp工具集是一家餐厅的不同服务方式，每种方式都有其独特的适用场景：

**llama-cli = 堂食服务**

你走进一家精致的餐厅，服务员热情地迎接你，为你安排座位。你可以直接与厨师交流，实时看到你的菜品如何制作，随时调整口味。这就是llama-cli的交互式模式——面对面的即时对话，适合探索、体验和调试。你想试试不同的温度参数？输入一个新值立即看到效果。想看看模型是如何"思考"的？推理内容实时显示在屏幕上。

**llama-server = 外卖平台**

你打开外卖APP，浏览菜单，下单，然后等待食物送到家门口。你不需要关心厨房里的具体操作，只需要一个标准化的订单接口。llama-server就像这样——它提供HTTP API，各种应用程序都可以调用它。你的手机APP、网站后台、自动化脚本，都能以相同的格式发送请求。OpenAI兼容的API意味着你可以用同样的代码调用GPT-4或本地的Llama模型。

**llama-quantize = 食材加工厂**

想象你有一批优质食材，但它们体积太大，不方便运输和储存。你需要一个加工厂，将它们压缩成便于分发的小包装，同时尽量保持品质。llama-quantize就是这个加工厂——它将动辄几十GB的模型压缩到几GB甚至更小，让普通用户也能在消费级硬件上运行大模型。imatrix就像是一位经验丰富的老师傅，知道哪些部分需要保留更多精度，哪些可以适当简化。

**llama-bench = 厨房效率测试**

餐厅老板想知道：新购入的烤箱比旧烤箱快多少？不同厨师做同一道菜的速度差异？哪些环节是瓶颈？llama-bench就是这套"效率测试系统"。它用标准化的流程测试各种配置，输出清晰的报告，帮助你做出优化决策。是增加GPU层数？还是换用更激进的量化？数据会告诉你答案。

就像一家成功的餐厅需要多种服务方式来满足不同顾客需求，llama.cpp也通过这四种核心工具覆盖了从个人体验到生产部署的完整场景。
...
| 方案 | 优点 | 缺点 | 选择 |
|-----|------|------|------|
| 独立实现 | 无依赖，体积小 | 代码重复，维护困难 | 否 |
| 复用server | 代码复用，功能一致 | 需要链接server库 | **是** |
| 提取公共库 | 架构清晰，职责明确 | 重构成本高，短期收益低 | 未来可能 |

**复用server代码的好处**：

1. **功能一致性**：CLI和Server使用完全相同的推理逻辑，同样的参数产生同样的结果
2. **维护简单**：修复bug只需改一处，不会出现在CLI修复了但Server没修复的情况
3. **功能丰富**：当Server支持新功能（如多模态、工具调用）时，CLI自动获得
4. **测试覆盖**：测试Server的代码同时也测试了CLI的底层逻辑

**代价**：
- CLI二进制体积稍大（链接了server代码）
- 架构上不那么"纯粹"

但考虑到开发效率和维护成本，这个权衡是值得的。实际上，很多现代工具都采用这种"CLI是本地版Server"的设计（如Docker、Kubectl等）。
...
// 如果集成到主程序：
// ./llama-cli --quantize input.gguf output.gguf Q4_K_M
// 
// 问题：
// - 增加了主程序复杂性
// - 增加了二进制体积
