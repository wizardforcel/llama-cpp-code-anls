...
- **无约束生成**（自由写作）：开放性问题，可以任意发挥，但可能离题万里。模型可能生成优美的散文，却不是你要的JSON格式。

- **语法约束**（填空题）：有明确的格式要求，必须在指定位置填入合适的内容。就像填写表格，每个字段都有严格的格式要求。

- **GBNF语法**（题目模板）：定义了填空的规则和可选项。它告诉模型：这里需要一个数字，那里需要一个字符串，括号必须成对出现。

例如，生成JSON就像做一道有严格格式的填空题：
```
姓名：____  年龄：____  邮箱：____
```
语法约束确保模型不会在"年龄"位置填入字符串，也不会忘记闭合引号。
...
| 特性 | 优点 | 缺点 |
|------|------|------|
| 完整GBNF | 表达力强，支持复杂语法 | 解析开销大 |
| 简化语法 | 解析快，适合简单约束 | 表达能力受限 |
| 预编译语法 | 运行时快 | 内存占用增加，初始化慢 |

### 17.6.2 约束严格度

- **严格约束**：保证输出格式100%正确，但可能降低生成质量（模型被迫选择次优token）
- **宽松约束**：允许更多灵活性，但需要后处理验证

llama.cpp选择严格约束路线，因为：
1. 结构化输出的正确性通常比流畅性更重要
2. 可以在语法设计上提供足够灵活性（如可选字段）

### 17.6.3 增量解析vs批量解析

| 方式 | 优点 | 缺点 |
|------|------|------|
| 增量解析 | 内存效率高，适合流式生成 | 状态管理复杂 |
| 批量解析 | 实现简单 | 需要预先生成完整序列 |

llama.cpp采用增量解析，因为流式生成是大模型推理的关键需求。
...
| 概念 | 一句话总结 |
|------|-----------|
| **GBNF** | llama.cpp的语法描述语言，基于EBNF |
| **语法解析器** | 递归下降解析器，将GBNF转为内部规则 |
| **下推自动机** | 跟踪解析状态，使用栈处理嵌套结构 |
| **候选过滤** | 根据语法规则拒绝不符合的token候选 |
| **增量解析** | 逐字符更新解析状态，支持流式生成 |
| **JSON Schema转换** | 自动将JSON Schema转为GBNF语法 |
| **懒加载语法** | 等待触发词后激活约束，支持工具调用 |

**语法约束的核心思想**：

语法约束是"边生成边检查"的模式。传统方法生成后验证，错误率高且需要重新生成；语法约束在生成过程中实时过滤，保证100%格式正确。这就像在填空时就检查答案格式，而不是交卷后才发现格式错误。

**下一步预告**：

在第18章，我们将探索高级生成技术——投机解码（Speculative Decoding）、前瞻解码（Lookahead Decoding）等加速技术，让模型生成更快更高效。
