...
想象你来到一家有10万道菜品的餐厅，每道菜都有一个"推荐分数"（logit）。采样算法就像是你选择菜品的策略：

- **贪心采样**：每次都点分数最高的那道菜——稳妥但缺乏惊喜，适合需要确定性的场合
- **随机采样**：完全随机选择——可能点到黑暗料理，但偶尔有意外惊喜
- **温度缩放**：调整"冒险程度"，温度高更愿意尝试低分菜品，温度低则保守
- **Top-K**：只从评分前K名的菜品中选择，平衡质量与多样性
- **Top-P**：从累计评分占比达到P%的菜品池中选择，动态调整候选池大小
- **重复惩罚**：避免连续点同一道菜，让餐桌更丰富多样

就像点菜需要根据不同的场合（商务宴请vs朋友聚会）选择不同策略，采样参数也需要根据任务类型（代码生成vs创意写作）进行调整。
...
1. **Softmax计算**：每次采样都需要，是主要开销
   - 优化：可在采样链末尾统一计算

2. **排序开销**：Top-K/Top-P需要排序
   - 优化：使用`std::nth_element`部分排序，O(n) vs O(n log n)

3. **内存访问**：惩罚采样需要访问历史token
   - 优化：使用哈希表缓存最近token统计

### 16.5.3 采样算法的局限性

1. **贪心陷阱**：局部最优不等于全局最优，可能导致生成陷入循环
2. **温度敏感**：不同模型对温度敏感度不同，需要实验调优
3. **上下文依赖**：固定参数难以适应所有上下文，可能需要动态调整
...
| 概念 | 一句话总结 |
|------|-----------|
| **贪心采样** | 总是选概率最高，确定性强但缺乏多样性 |
| **温度缩放** | 控制"冒险程度"，低温保守高温随机 |
| **Top-K** | 固定保留前K个候选，简单高效 |
| **Top-P** | 动态保留累积概率达P%的候选，自适应推荐 |
| **Min-P** | 相对于最高概率设置阈值，避免极端情况 |
| **Typical** | 基于信息熵选择"典型"token，更自然 |
| **重复惩罚** | 降低已出现token的概率，避免单调 |
| **采样器链** | 模块化组合多种采样策略 |
...
在第17章，我们将探索语法约束（Grammar Constraints）——如何使用GBNF（GGML BNF）语法强制模型生成符合特定格式的输出，如JSON、SQL等结构化数据。
