...
想象 `llama_vocab` 是一家精通百种语言的翻译局。词表（Vocabulary）就是它的翻译字典，每个 token 是一个"词汇条目"，包含文本内容、分数（频率）和属性，不同模型的字典大小从 32K 到 200K 不等。分词（Tokenization）就是编码过程——将人类可读的文本转换为模型可理解的 token ID，类似摩斯电码编码，比如"Hello"可能被分为 [15496, 11]（对应"Hell"+"o"两个 token）。

反过来，反分词（Detokenization）是解码过程，将 token ID 序列转换回人类可读的文本，类似摩斯电码解码。而在正式编码之前，预分词（Pre-tokenization）会先对文本进行预处理——按正则表达式切分文本，处理 Unicode、空格、标点等复杂情况。

就像翻译局需要精确处理每种语言的特性——英语的缩写、中文的表意文字、日语的假名——分词器需要处理不同模型的词表格式和分词策略。同一个句子在不同模型的分词器下可能产生完全不同的 token 序列，这直接影响模型的理解和生成效果。
...
| 概念 | 解释 |
|------|------|
| llama_vocab | 词表管理类，封装 tokenize（编码）和 detokenize（解码）功能 |
| BPE | 字节对编码，从字节级开始迭代合并最频繁的字符对来构建词表 |
| SPM | SentencePiece 分词，基于 Viterbi 动态规划算法找出最优 token 序列 |
| 预分词 | 用正则表达式将文本切分为单词候选，不同模型有不同的切分规则 |
| 特殊 Token | BOS（开始）、EOS（结束）、UNK（未知）等控制标记，管理序列边界 |
| 反分词 | 将 token 序列还原为人类可读文本，处理 BPE 字节映射和 SPM 空格标记 |

**下一章预告**：

下一章中，我们将学习采样策略，理解如何让模型生成多样化且高质量的文本。
