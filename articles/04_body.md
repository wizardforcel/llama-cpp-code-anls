...
想象 GGML 的计算图是一家智能工厂的生产流水线。在这个流水线上，张量就像传送带上流转的物料箱，里面装着待加工的数值；各种运算操作则是沿着传送带分布的加工机器——加法机负责合并物料，乘法机按比例调配成分，混合机则执行复杂的配方工艺。内存池相当于仓库里的货架，这些货架在生产开始前就已经规划好了存储空间，每道工序的中间产物都可以暂时存放在上面。

工厂里还有几种精巧的管理技巧：in-place 优化就像是"原地改造"——不搬运物料箱，直接在原位完成加工，省去了来回搬动的开销。view 机制则是"贴标签"的艺术，多个箱子可以共享同一批物理货物，只是贴上不同的标签来区分各自的用途。而拓扑排序则是工厂的生产调度中心，它确保每一批原材料都先经过前道工序处理，再流入后道工序，任何环节都不会因为依赖关系没准备好而空转等待。

就像工厂经理需要精心安排每台机器的位置和物料流转路径一样，GGML 的计算图也需要精确规划每个算子的内存使用和计算顺序。一条优秀的流水线能最大限度地减少物料搬运、降低仓库占用；同理，一个经过优化的计算图能够最大限度地复用内存、减少数据拷贝。
...
本章系统介绍了 GGML 的核心算子实现。`ggml_add/mul` 实现元素级运算，要求输入形状完全相同，支持多线程并行加速。`ggml_mul_mat` 实现矩阵乘法 GEMM，这是深度学习中的核心运算，也是计算量最大的部分。`ggml_silu` 实现了 Swish 激活函数，是 LLM 的首选激活函数，计算公式为 SiLU(x) = x * sigmoid(x)。`ggml_rms_norm` 实现 RMS 归一化，这是 LayerNorm 的简化版本，被 LLaMA 等模型采用。`ggml_rope` 实现旋转位置编码，用于向模型注入位置信息，支持长度外推。`ggml_view_*` 系列函数提供视图操作，实现零拷贝的形状变换，多个视图共享同一块内存。`ggml_*_inplace` 原地操作将结果直接写入输入内存，可节省约 50% 的内存空间。

本章核心要点包括：运算分三阶段进行——创建阶段定义图结构，构建阶段进行拓扑排序，执行阶段完成实际计算；类型分发机制在运行时根据张量类型分发到特定实现；元素级运算天然可并行，使用多线程加速；视图操作、原地操作和内存复用是推理性能优化的关键。

本章我们一起学习了以下概念：

| 概念 | 解释 |
|------|------|
| 元素级运算 | 对两个张量的对应元素执行相同操作，天然支持多线程并行 |
| 矩阵乘法 GEMM | 深度学习最核心运算，占推理时间60%以上，采用分块和多后端加速 |
| SiLU 激活函数 | Swish/SiLU 是 LLM 首选激活，平滑非单调且比 ReLU 表达力更强 |
| RMS 归一化 | LayerNorm 简化版，无需减均值，减少约30%计算量 |
| RoPE 旋转位置编码 | 通过旋转矩阵注入位置信息，支持相对位置编码和长度外推 |
| 内存复用优化 | 视图（零拷贝）、原地操作（节省50%内存）、生命周期分析（共享内存） |

下一章中，我们将学习 GGUF 模型格式——理解模型是如何存储和加载的，以及不同量化方案对性能和质量的影响。
